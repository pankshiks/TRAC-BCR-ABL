...
//('Specifying tick interval for a logarithmic scale is undefined. Remove any calls to SetTextLabelStart() or SetTextTickInterval() on the logarithmic scale.');
	...
	if( $scale->type == "y" ) {
        ...
        {
            $new_l=1;  
            $dd=round(log10($start));                                              
            if(round(log10($start))<0)
            {
                for($i=1; $i<=abs($dd); $i++)
                {
                $new_l=(1/10)*$new_l;
                ...
                if($new_l=='0.1')   
                {
                $label='MMR   0.1';//represent 0.1
                $this->maj_ticks_label[0]=$label;                
                }                
                else if($i=='2')
                {                
                $label='MR4  0.01';//represent 0.01
                $this->maj_ticks_label[0]=$label;                
                }//0.0032  == 10^2.49                
                else if($i=='3') 
                {   
                //$this->maj_ticks_label[$i]='10^'.round(log10('-2.49'));                    
                $label='MR5 0.001';//represent 0.001
                $this->maj_ticks_label[0]=$label;                    
                } 
                else                                
                $this->maj_ticks_label[0]=$new_l;  //represent 0.0001   $this->maj_ticks_label[0]=$new_l.'MM5'; 
                } 
                 }
                else if(round(log10($start))==0)
                 {
              //  for($k=0; $k<=abs($dd2); $k++)
                //$new_2=(1/10)*$new_2;                                
                $this->maj_ticks_label[0]='CCYR 1';  
                } 
      ...
      //
		...
		if( $count % 10 == 0 ) {
            ...
            $new_2=1;  
            $dd2=round(log10($nextMajor));                                              
            if(round(log10($nextMajor))<0)
            {
                for($k=1; $k<=abs($dd2); $k++)
                {
                                        
                $new_2=(1/10)*$new_2;                                
                //$this->maj_ticks_label[$i]=$new_2.'k';  
                if($new_2=='0.1')   
                {             
                $label='MMR   0.1';//represent 0.1
                $this->maj_ticks_label[$i]=$label;                
                }                
                else if($k=='2')
                {                
                $label='MR4  0.01';//represent 0.01
                ...
                $this->maj_ticks_label[$i]=$new_2;
}
                
                } 
               else if(round(log10($nextMajor))==0)
                 {
              //  for($k=0; $k<=abs($dd2); $k++)
                //$new_2=(1/10)*$new_2;                                
                $this->maj_ticks_label[$i]='CCYR -   1';// represent 1 
                } 
                else
                {    
                 for($k=1; $k<=abs($dd2); $k++)
            ...
            /*
             $this->maj_ticks_label[1]='0.001';
                $this->maj_ticks_label[2]='0.01';
                $this->maj_ticks_label[3]='0.1';
                $this->maj_ticks_label[4]='1';
                $this->maj_ticks_label[5]='10';
                $this->maj_ticks_label[6]='100';
                $this->maj_ticks_label[7]='1000';       
                */
            }
		    ++$i;						
		    $nextMajor *= 10;
